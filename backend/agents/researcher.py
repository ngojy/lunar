...
def researcher_node(state: AgentState) -> AgentState:
    ...
    force_web = state.get("metadata", {}).get("web_search", False)

    if not force_web:
        # Let LLM decide if search is needed
        check_response = _timed_invoke(
            _llm,
            [
                SystemMessage(content="Does this task require searching the internet for current information? Reply with YES or NO only."),
                HumanMessage(content=f"Task: {state['task']}"),
            ],
            "Checking if web search needed",
        )
        needs_search = "YES" in check_response.content.upper()
        if not needs_search:
            # Answer from model knowledge directly
            response = _timed_invoke(
                _llm,
                [
                    SystemMessage(content="Answer this question using your knowledge. Be concise and accurate."),
            ...
            return {
                **state,
                "research_results": existing + [response.content.strip()],
                "messages": [{"role": "researcher", "content": "Answered from knowledge."}],
            }
